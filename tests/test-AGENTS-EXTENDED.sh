...
set -uo pipefail
F="$(cd "$(dirname "$0")/.." && pwd)/AGENTS-EXTENDED.md"
...
echo "──────────────────────────────────────"
assert "No override flow section" "! grep -q 'Override flow' '$F'"
assert "Has TDD no override" "grep -q 'no override' '$F'"
assert "Uses v6" "grep -q 'commit-msg v6\|TDD gate' '$F'"
